...
unless File.exist?("/home/dev/.ssh")
...
File.chmod(0600, "/home/dev/.ssh/authorized_keys") if File.exist?("/home/dev/.ssh/authorized_keys")
File.chmod(0600, "/home/dev/.ssh/config") if File.exist?("/home/dev/.ssh/config")

puts "Installing SSHD config"

unless File.exist?("/shared/ssh/sshd_config")
  puts "Copying sample SSHD config since none exists"
  FileUtils.copy("/etc/ssh/sshd_config.original", "/shared/ssh/sshd_config")
end

File.unlink("/etc/ssh/sshd_config") if File.exist?("/etc/ssh/sshd_config")

unless File.exist?("/shared/ssh/config")
...
if !File.exist?(etc_sshd_file_path) || File.size(shared_sshd_file_path) != File.size(etc_sshd_file_path)
...
if File.exist?(compose_environment_file_path)
  ...
  FileUtils.touch(profile_path) unless File.exist?(profile_path)
